...
def denoise_seq_fastdvdnet(seq, noise_std, temp_psz, model_temporal):
    ...
    Args:
        seq: Tensor. [numframes, 1, C, H, W] array containing the noisy input
            frames
    ...
    for fridx in range(numframes):
        ...
        if not inframes:
            ...
            for idx in range(temp_psz):
                # handle border conditions, reflect
            ...
            relidx = min(
                fridx + ctrlfr_idx,
                -fridx + 2 * (numframes - 1) - ctrlfr_idx,
            )
            inframes.append(seq[relidx])

        inframes_t = \
            torch.stack(inframes, dim=0) \
            .contiguous() \
            .view((1, temp_psz * C, H, W)) \
            .to(seq.device)
